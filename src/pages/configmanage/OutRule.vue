<template lang="html">
  <div class="outRule">
    <div class="chooseRule">
      <table>
        <tr height="40px">
          <!-- <td class="blueYaHei20">出库分拣规则</td> -->
          <td width="78%"></td>
          <td>
      ...
      <table>
        <tr height="40px">
          ...
          <td class="redYaHei18" align="center">
            出库方式必选，客户规则和供应商规则二选一，商品种类规则和商品分类规则二选一
          </td>
        </tr>
      </table>
      <table>
    ...
    <div class="ruleList" width="30%" >
      <table>
        <tr height="40px">
          <td width="10%"></td>
          <td class="blueYaHei18" width="40%">出库分拣规则顺序</td>
          <td width="40%"></td>
  ...
  methods:{
    ...
    generateRuleRecord(){
      if(this.chooseRuleList.length > 0){
        let choosenOutWay = false //未选择出库方式
        this.chooseRuleList.forEach((item)=>{
          if(item.ruleNameEng == 'outWay'){
            choosenOutWay = true
          }
        ...
        if(choosenOutWay){
          let url = sessionStorage.http+"/generateRule?recordType=6&rules="+this.generateRuleList+"&loginStatus="+`${sessionStorage.loginStatus}`
