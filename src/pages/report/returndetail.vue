    ...
    <Form :label-width="90" inline style="text-align:left;">
      <div>
        ...
        <Form-item label="检验批号" >
          <Input v-model="searchCommodityCategory.checknumber" @on-change="searchItemTypeByCondition"
          placeholder="请输入"/>
        </Form-item>

        <Form-item label="质量状态" >
          <Select v-model="searchCommodityCategory.qualitystate" clearable placeholder="请选择"
           @on-change="searchItemTypeByCondition">
           <Option value="Q">待检</Option>
    ...
    export default {
      created(){
        let param = {loginStatus:sessionStorage.loginStatus, curPage : 1,pageSize : 20}
          axios.post(sessionStorage.http+`/getreturndataByCondition`,param)
            .then((result) => {
                ...
                columns11: [
                  {
                      ...
                      render:(h,params)=>{
                        let isstart = '';
                        let strs = [];
                        strs = params.row.createdate.split("-")
                          isstart = strs[0];
        ...
        methods:{
          searchItemTypeByCondition(){
            ...
            axios.post(sessionStorage.http+`/getreturndataByCondition`,this.searchCommodityCategory)
              .then((result) => {
                this.data10 = result.data.data
                this.total = result.data.length
              })
          ...
          exportcheckreport(){
            let param = "loginStatus="+`${sessionStorage.loginStatus}`+ "&goodsid="+this.searchCommodityCategory.goodsid+
            "&stockNumber="+this.searchCommodityCategory.stockNumber+
            "&goodsdescribe="+this.searchCommodityCategory.goodsdescribe+
            "&checkNumber="+this.searchCommodityCategory.checkNumber+"&goodstype="+this.searchCommodityCategory.goodstype

            window.open(sessionStorage.http+"/exportreturndetailreport?"+param)
          },
        }
    }
</script>
