...
export default {
    ...
    return {
      ...
      tableColumns: {
         card: [
           ...
           {title: '出库单号',key: 'outId',align: 'center'},
           {title: '托盘号',key: 'trayNo',width:250, align: 'center'},
           {title: '货位', key: 'origstorageid',width:250,align: 'center'},
    ...
    if(typeof(WebSocket) == "undefined") {
        ...
        console.log("您的浏览器支持WebSocket");
          ...
          //var socketIp = 'http://192.168.17.50:8088/websocket/taskIn';
            ...
            //1.0当入库任务数据更新时更新页面
            var socket;
            var socketIp = 'http://localhost:8088/websocket/taskOut';
            ...
            socket.onopen = function() {
                console.log("taskOut对应的websocket已打开");
            };
            //获得消息事件
            ...
            socket.onclose = function() {
                console.log("taskOut对应的websocket已关闭");
            };
            //发生了错误事件
            socket.onerror = function() {
                alert("taskOut对应的websocket发生了错误");
                //此时可以尝试刷新页面
            }
            //离开页面时，关闭socket
            ...
            var socket2;
            var socketIp2 = 'http://localhost:8088/websocket/taskOutError';
            ...
            socket2.onopen = function() {
                console.log("taskOutError对应的websocket已打开");
            };
            //获得消息事件
            ...
            socket2.onclose = function() {
                console.log("taskOutError对应的websocket已关闭");
            };
            //发生了错误事件
            socket2.onerror = function() {
                alert("taskOutError对应的websocket发生了错误");
                //此时可以尝试刷新页面
            }
            //离开页面时，关闭socket
            //jquery1.8中已经被废弃，3.0中已经移除
  ...
  methods:{
    //...mapMutations('layout', ['modifyTestData']),
    // checkTask(){
    //   axios.post(localStorage.http +`/checkTask`)
    //   .then((res) =>{
    //
    //     if(res.data == "true"){
    //       this.searchTask()
    ...
    //页面初始化，查询数据
    searchTask(){
      axios.post(localStorage.http +`/searchTaskOut`)
      .then((res) =>{
