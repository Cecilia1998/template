<template lang="html">
  <div class="addNotice">
    <Modal v-model="showAddOutNotice" title="新增出库通知单" :mask-closable='false' footer-hide okText="确定" cancel-text='取消' @on-ok="submit" @on-cancel="cancel" :loading="loading" width='90%' style="height:500px;">
          ...
          <td>
            <div class="" style="position:relative">
              <span class="outBox">
                ...
                <span class="blackLabel">移动类型 <span class="inputBox">
                  <Select v-model="formFilter.movetype"  filterable remote   style="width:160px;">
                    <Option value=""></Option>
                    ...
                    <Option value="CKT05">其它出库</Option>
                  </Select>
                </span>
              </span>
              </span>
              <span class="outBox">
                ...
                <span class="inputBox">
                  <DatePicker v-model="formFilter.predictOutDateD" @on-change="predictOutDateChanged" type="date" placeholder="请选择" class="dateInput" style="width:160px;"></DatePicker>
                  <!-- -
            ...
            <div class="" style="position:absolute;bottom:15px;right:50px">
              ...
              <span class="blackLabel" v-if="false" >行明细</span>
                <Button shape="circle" @click="addNoticeDetail" type="primary"  style="width:120px;height:28px;padding:0px;border-radius:14px;background-color:#5aa5ff;"  >添&nbsp;&nbsp;加</Button>
              <Button shape="circle" @click="submitNewOutNotice"  type="primary"  style="width:120px;height:28px;padding:0px;border-radius:14px;background-color:#5aa5ff;"  >提&nbsp;&nbsp;交</Button>
...
export default {
  ...
  data () {
    return {
      showAddOutNotice:false,//modal显示标记
      ...
      tableColumns: {//表格数据
        data:[
         ...
         card: [
                  ...
                  {title: '物料/产品号',key: 'materialNo',width:200, align: 'center',
                   render: (h, params) => {
                     return h('div',[
                       h('Input',{
                         props: {
                       ...
                       on:{
                           ...
                           'on-search':(materialNo)=> {
                             let could = true
                             for (var i = 0; i < this.noticeDetail.length; i++) {
                               if (this.noticeDetail[i].materialNo == materialNo) {
                  ...
                  {title: '物料/产品描述',key: 'materialDesc', width:200,align: 'center'},
                  {title: '库存批号', key: 'stockBatch',width:150,align: 'center'},
                  {title: '检验批号', key: 'checkBatch',width:150,align: 'center'},
                  {title: '质量状态', key: 'qualityStatus',width:100,align: 'center'},
                  {title: '出库数量', key: 'quantity',width:100,align: 'center',
                  render: (h, params) => {
                     return h('div',[
                       h('InputNumber',{
                         props: {
                       ...
                       on:{
                           ...
                           'on-blur':()=>{
                             if (params.row.quantity > params.row.canQuantity) {
                               this.$Message.error("出库数量不能大于实际库存数量")
                               params.row.quantity = params.row.canQuantity
                             }else {
                               this.noticeDetail[params.row.rowid-1].quantity = params.row.quantity
                             }
                  ...
                  // {title: '件数', key: 'trayno',width:100,align: 'center'},
                  {title: '包装规格', key: 'packageType',width:120,align: 'center'},
  ...
  methods:{
    ...
    addNoticeDetail(){
      let detail = {materialNo:'',quantity:null,canQuantity:0}
      this.tableColumns.data.push(detail)
    },
    submitNewOutNotice(){
      if (this.noticeDetail.length > 0) {
        this.noticeDetail.push(this.formFilter)
        axios.post(sessionStorage.http+'/outNotice/submitNewOutNotice?loginStatus='+`${sessionStorage.loginStatus}`,this.noticeDetail)
        .then((res) =>{
          if (res.data.length > 0) {
            for (var i = 0; i < res.data.length; i++) {
              this.noticeDetail.push(res.data[i])
            }
    ...
    getOutMaterial(materialNo,params){
      axios.post(sessionStorage.http+'/outNotice/getOutMaterial?loginStatus='+`${sessionStorage.loginStatus}`+'&materialNo='+materialNo,'')
      .then((res) =>{
        if (res.data.length > 0) {
          for (var i = 0; i < res.data.length; i++) {
            this.noticeDetail.push(res.data[i])
    ...
    deleteDetail(){//删除选中的行
      ...
      for(let i = this.noticeDetail.length - 1; i >= 0 ; i--){
        if(this.noticeDetail[i]._checked){
          this.noticeDetail.splice(this.noticeDetail[i].rowid-1,1)
        }
      }
      this.total = this.noticeDetail.length
      this.trimRowid()
    ...
    onSelect(selection,row){
      this.noticeDetail[row.rowid-1]._checked = true
      this.hasSelected = 'true'
    },
    onSelectCancel(selection,row){
      this.noticeDetail[row.rowid-1]._checked = false
      this.getHasSelected()
    },
    onSelectAll(selection){
      for(var i = 0;i < this.noticeDetail.length;i++){
        this.noticeDetail[i]._checked = true
      }
      // for(var i = 0;i<selection.length;i++){
      //   this.noticeDetail[selection[i].rowid-1]._checked = true
      // }
      this.hasSelected = 'true'
    },
    onSelectAllCancel(selection){
      for(var i = 0;i < this.noticeDetail.length;i++){
        this.noticeDetail[i]._checked = false
      }
      this.hasSelected = 'false'
    },
    ...
    trimPageData(){
      ...
      for (var i = startIndex; i < this.noticeDetail.length && i <endIndex; i++) {
        this.tableColumns.data.push(this.noticeDetail[i])
      }
    },
    show(){
      this.showAddOutNotice = true
    },
    ...
    queryData(){//查询数据
      axios.post(sessionStorage.http+'/getaddNotice?loginStatus='+`${sessionStorage.loginStatus}`,this.formFilter)
      .then((res) =>{
        console.log(res)
