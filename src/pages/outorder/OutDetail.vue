<template lang="html">
  <div class="outDetail">
    <Modal v-model="showOutOutDetail" title="出库单-详情" :mask-closable='false' footer-hide okText="确定" cancel-text='取消'  :loading="loading" width='90%'>
      <Form ref="formFilter" :model="formFilter" :label-width="80" inline style="text-align:left;">
        ...
        <tr>
          <td>
            ...
            <span class="outBox">
              ...
              <span class="inputBox">
                <span>{{outOrder.outDate}}</span>
              </span>
            </span>
            <span class="outBox">
              <span class="blank" style="width:20px;">&nbsp;</span>
              <span class="blackLabel">出库总件数</span>
              <span class="inputBox">
                <span>{{outOrder.num}}</span>
              </span>
            </span>
            <span class="outBox">
              <span class="blank" style="width:20px;">&nbsp;</span>
              <span class="blackLabel">出库总重量</span>
              <span class="inputBox">
                <span>{{outOrder.quantity}}</span>
              </span>
            </span>
            <!-- <span class="outBox">
              <span class="blank" style="width:20px;">&nbsp;</span>
              ...
              <span class="inputBox">
                <span v-if="outOrder.outcomingNotice != '' ">{{outOrder.outcomingNotice}}</span>
                <span v-if="outOrder.outcomingNotice == '' ">无通知单</span>
              </span>
            </span>
            <span class="outBox">
              <span class="blank" style="width:20px;">&nbsp;</span>
              ...
              <span class="inputBox">
                <span>{{outOrder.orderbillnum}}</span>
              </span>
            </span> -->
            <!-- <span class="outBox">
              <span class="blank" style="width:20px;">&nbsp;</span>
              ...
              <span class="inputBox">
                <span>{{outOrder.movetypeTxt}}</span>
              </span>
            </span> -->
            <span class="outBox">
              <span class="blank" style="width:20px;">&nbsp;</span>
              ...
              <span class="inputBox">
                <span v-if="outOrder.orderType == 'CKT05' ">一般出库</span>
                <span v-if="outOrder.orderType == 'CKT04' ">拼盘出库</span>
              </span>
            </span>
            <span class="outBox">
              <span class="blank" style="width:20px;">&nbsp;</span>
            ...
            <span class="outBox">
              ...
              <span class="inputBox">
                ...
                <span v-if="outOrder.status == '0' ">申请</span>
                <span v-if="outOrder.status == '1' ">接收</span>
                <span v-if="outOrder.status == '2' ">出库中</span>
                <span v-if="outOrder.status == '3' ">出库完成</span>
              </span>
            </span>
            ...
            <!-- <div class="" style="position:absolute;bottom:15px;right:50px">
                ...
                <Button shape="circle" @click="confirmOut" type="primary"  style="width:120px;height:28px;padding:0px;border-radius:14px;background-color:#5aa5ff;"  >确认出库</Button>
                <Button shape="circle" @click=""  type="primary"  style="width:120px;height:28px;padding:0px;border-radius:14px;background-color:#5aa5ff;"  >取消确认</Button>
              <Button shape="circle" @click=""  type="primary"  style="width:120px;height:28px;padding:0px;border-radius:14px;background-color:#5aa5ff;"  >提&nbsp;&nbsp;交</Button>
        ...
        <tr>
          <td align="right">
            <Button shape="circle" @click="confirmOut" v-if="false" type="primary"  style="width:120px;height:28px;padding:0px;border-radius:14px;background-color:#5aa5ff;"  >确认</Button>
      ...
      <Table v-show="showTable != 'pinpan'" height=500 border stripe  :loading="loading" ref="selection" :columns="tableColumns.card" :data="tableColumns.data" @on-selection-change="onSelectionChange" @on-select="onSelect" @on-select-cancel="onSelectCancel" @on-select-all="onSelectAll" @on-select-all-cancel="onSelectAllCancel" ></Table>
      <Table v-show="showTable == 'pinpan' " height=500 border stripe  :loading="loading" ref="selection" :columns="pinpan.card" :data="tableColumns.data" @on-selection-change="onSelectionChange" @on-select="onSelect" @on-select-cancel="onSelectCancel" @on-select-all="onSelectAll" @on-select-all-cancel="onSelectAllCancel" ></Table>
...
export default {
  ...
  data () {
    return {
      ...
      showOutOutDetail:false,//modal显示标记
      ...
      pinpan:{
        card:[
          ...
          {title: '明细单号',key: 'outOrderDetailId',width:250, align: 'center'},
          {title: '物料号', key: 'materielId',width:120,align: 'center'},
          {title: '物料名称', key: 'materielName',width:120,align: 'center'},
          {title: '批次号', key: 'batchNo',width:120,align: 'center'},
          ...
          {title: '出库件数', key: 'num',width:120,align: 'center'},
        {title: '出库重量', key: 'weight',width:120,align: 'center'},
        ...
        {title: '计量单位', key: 'unit',width:120,align: 'center'},
          ...
          {title: '出库仓库',key: 'warehouseName', width:200,align: 'center'},
           {title: '出库库区',key: 'districtName', width:200,align: 'center'},
        ...
        data:[
         ...
         card: [
          ...
          //  },
           ...
           //       return h('span',(params.row.quantity + parseFloat(params.row.standby2)).toFixed(3))
           //
           //   }
           // },
           {title: '质量状态', key: 'qualityStatus',width:120,align: 'center',
  ...
  methods:{
    ...
    show(outOrder){
      ...
      console.log("dbclick查询数据")
      console.log(outOrder)
      // if(outOrder.standby2 == 'N'){
      //   this.showOutOutDetail = false;
      //   this.$Message.error("该出库单为实物不出库,明细无数据")
      //
      // }
      if (this.outOrder.moveType == 'CKT04' || this.outOrder.movetypeTxt == '拼盘出库') {
        this.showTable = 'pinpan'
      }else {
        this.showTable = 'qita'
      }
      // if(this.outOrder.orderSource == 'SAP'){
      //   if(this.tableColumns.card[9].key != 'noticeQuantity'){
      //     this.tableColumns.card.splice(9,0,this.noticeQuantity)
      //   }
      // }else if (this.outOrder.orderSource == 'WMS'){
      //   if(this.tableColumns.card[9].key == 'noticeQuantity'){
      //     this.tableColumns.card.splice(9,1)
      //   }
      // }

      this.showOutOutDetail = true
      this.queryOutDetail()
    },
    queryOutDetail(){//查询数据
      axios.post(sessionStorage.http+'/queryOutDetail?loginStatus='+`${sessionStorage.loginStatus}`,this.outOrder)
      .then((res) =>{
        console.log(res)
    ...
    queryData(){//查询数据
      axios.post(sessionStorage.http+'/getoutDetail?loginStatus='+`${sessionStorage.loginStatus}`,this.formFilter)
      .then((res) =>{
        console.log(res)
